...
-(void)processImage{
    ...
    switch (self.processType) {
        case 1:
        {
            double h = _image.rows/4.0;
            double centerX = _image.cols/2.0;
            double centerY = _image.rows/2.0;
            cv::line(_image, cv::Point(centerX, centerY),
                     cv::Point(centerX, centerY + h),
                     Scalar(255,255,255,255));
            
            // left sise Lines
            double leftBackUpX = _image.cols/2.0 - h*sqrt(3.0)/2.0;
            double leftBackUpY = _image.rows/2.0 - h/2.0;
            cv::line(_image, cv::Point(centerX, centerY), cv::Point(leftBackUpX, leftBackUpY), Scalar(255,255,255,255));
            
            double leftBackDownX = _image.cols/2.0 - h*sqrt(3.0)/2.0;
            double leftBackDownY = _image.rows/2.0 + h/2.0;
            cv::line(_image, cv::Point(centerX, centerY + h), cv::Point(leftBackDownX, leftBackDownY), Scalar(255,255,255,255));
            
            // vertical left back line
            cv::line(_image, cv::Point(leftBackDownX, leftBackDownY),cv::Point(leftBackUpX, leftBackUpY), Scalar(255,255,255,255));
            
            
            // Right side lines
            double rightBackUpX = _image.cols/2.0 + h*sqrt(3.0)/2.0;
            double rightBackUpY = _image.rows/2.0 - h/2.0;
            cv::line(_image, cv::Point(centerX, centerY), cv::Point(rightBackUpX, rightBackUpY), Scalar(255,255,255,255));
            
            double rightBackDownX = _image.cols/2.0 + h*sqrt(3.0)/2.0;
            double rightBackDownY = _image.rows/2.0 + h/2.0;
            cv::line(_image, cv::Point(centerX, centerY + h), cv::Point(rightBackDownX, rightBackDownY), Scalar(255,255,255,255));
            
            // veritcal right back line
            cv::line(_image, cv::Point(rightBackDownX, rightBackDownY), cv::Point(rightBackUpX, rightBackUpY), Scalar(255,255,255,255));
            
            double centerUpX = centerX;
            double centerUpY = centerY - h;
            cv::line(_image, cv::Point(rightBackUpX, rightBackUpY), cv::Point(centerUpX, centerUpY), Scalar(255,255,255,255));
            cv::line(_image, cv::Point(leftBackUpX, leftBackUpY), cv::Point(centerUpX, centerUpY), Scalar(255,255,255,255));
